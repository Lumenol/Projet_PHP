...
function creerEmploye($idemploye, $mdpemploye, $categorie) {
	$connexion = getConnect ();
	$requete = "if not exists (select * from employe where identifiant = '$idemploye') begin  insert into employe VALUES('$idemploye', '$mdpemploye', '$categorie') end ";
	//$requete="INSERT INTO `garage`.`employe` (`identifiant`, `mdp`, `categorie`) VALUES ('$idemploye', '$mdpemploye', '$categorie')";
	$resultat = $connexion->exec ( $requete );
}
